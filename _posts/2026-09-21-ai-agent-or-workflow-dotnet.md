...
Усі чотири працюють і на демо дають однаковий результат. Різниця з'являється в рахунку за токени, у часі відповіді, у тому, що станеться після невдалого релізу моделі, і в тому, скільки коштує розібратися, чому система вирішила саме так.

Нижче одна задача, чотири її реалізації на .NET 10 і decision framework, який можна перенести на вашу задачу.
...
Переваги очевидні: мілісекунди, нуль токенів, повна відтворюваність, тести без моків, повний офлайн. Для більшості внутрішніх процесів цього достатньо.
...
Можна дописати ще двадцять слів у словник. Через місяць з'явиться двадцять перша фраза. Саме тут модель має сенс, але тільки для читання тексту. Рішення лишається за кодом.
...
Додався один виклик моделі, решта коду не змінилася.
...
`AgentWorkflowBuilder` створює handoff-tools автоматично, тому front desk не має жодного бізнес-tool: єдине, що він може зробити - передати тикет далі. Цю межу тримає код, промпт тут нічого не вирішує. Billing-спеціаліст не бачить `search_known_issues`, platform-спеціаліст не бачить `get_refund_policy`.
...
Різниця між 1 і 3 викликами моделі - це не 3x до рахунку. Кожен наступний виклик несе всю попередню історію разом з описами tools, тому вартість зростає швидше за кількість викликів. Помножте на кількість тикетів за день і на кількість повторних запусків після невдалого парсингу.
...
Політика захищає рішення, але не те, як модель прочитала текст.
...
**Чи можна записати правило?** Якщо так, пишіть функцію. Модель не потрібна там, де є таблиця відповідностей або регулярний формат. Для більшості задач функція так і залишиться остаточним рішенням.
...
Вивід моделі перевіряється перед використанням. Типізований результат плюс перевірка діапазонів, довідників і меж. Розбіжності між моделлю й політикою рахуються в метриках як звичайна подія.

Tools мають мінімальні права. Читання окремо від запису, гроші й розсилка - через людину. У прикладі всі tools read-only, і так зроблено свідомо.
...
Питання «агент чи workflow» зводиться до того, скільки control flow ви віддаєте моделі і скільки за це платите. Для більшості продуктових задач відповідь - workflow: модель читає текст, а порядок кроків і рішення залишаються в коді. Агента варто брати, коли маршрут справді невідомий заздалегідь, і тоді одразу з валідацією, лімітами та трасуванням. Multi-agent потрібен для окремих меж доступу, контекстів або паралелізму, архітектурна симетрія тут не аргумент.
