...
Збирання сміття (`Garbage Collection, GC`) - це автоматичний механізм керування пам'яттю, який звільняє розробників від необхідності вручну виділяти та звільняти пам'ять. Саме цим .NET відрізняється від мов, де пам'яттю керують вручну.
...
.NET GC розподіляє об'єкти за поколіннями. Їх три:
...
У цьому прикладі вільна пам'ять фрагментована на кілька маленьких шматків. Загалом вільної пам'яті може вистачати на новий об'єкт, але жоден окремий фрагмент не вміщує великий об'єкт.

У результаті пам'ять використовується неефективно, новий великий об'єкт важко розмістити, а програма працює повільніше.
...
Тепер усі вільні блоки об'єднані в один, і в ньому поміститься новий об'єкт, навіть великий.

### Як працює ущільнення в .NET

Ущільнення в .NET проходить у кілька кроків. Спочатку `GC` визначає, які об'єкти залишаються досяжними в пам'яті. Потім він створює план переміщення цих живих об'єктів таким чином, щоб усунути прогалини ("дірки") в пам'яті. Згідно з планом, `GC` копіює живі об'єкти у нові, послідовні місця в пам'яті. Після переміщення збирач сміття оновлює всі посилання на ці об'єкти, щоб вони вказували на нові адреси пам'яті. Нарешті, система звільняє стару пам'ять, де раніше знаходились об'єкти, роблячи її доступною для нових алокацій.

Для різних частин керованої кучі ущільнення працює по-різному.
У `Small Object Heap` (SOH) воно виконується під час кожного циклу збору сміття, тож пам'ять під малі об'єкти використовується щільно.
`Large Object Heap` (LOH) історично не ущільнювалася, бо переміщувати великі об'єкти дорого. Через це `LOH` часто страждає від фрагментації.
...
Найменше навантаження на GC дає пам'ять, яку ви взагалі не виділяли. Для частих алокацій беріть пули об'єктів (`ObjectPool<T>`), не допускайте боксингу типів-значень, а рядки в циклах збирайте через `StringBuilder`, а не конкатенацією. Для роботи з блоками пам'яті без копіювання є `Span<T>` і `Memory<T>`.

З великими об'єктами обережніше: не створюйте тимчасових великих масивів і колекцій, подумайте, чи не розбити великий об'єкт на кілька менших, а тимчасові великі масиви беріть з `ArrayPool<T>`.

Некеровані ресурси завжди треба звільняти, тому IDisposable реалізуйте правильно і використовуйте для таких об'єктів шаблон using. `GC.Collect()` у більшості випадків викликати не варто, лише в особливих ситуаціях, наприклад після великих операцій з пам'яттю.

Для кешу, який GC може очистити при нестачі пам'яті, підходить WeakReference. І уникайте циклічних посилань: GC їх обробляє, але вони можуть затримати звільнення пам'яті.
...
`GC` знімає з вас ручне керування пам'яттю, але не відповідальність за те, як ця пам'ять використовується. Якщо розуміти покоління, фази збору та ущільнення, легше спроєктувати систему так, щоб вона не страждала від фрагментації і працювала стабільно навіть під великим навантаженням.

На практиці це насамперед означає не робити зайвих алокацій, особливо великих об'єктів у LOH, і обрати режим `GC` під свій сценарій (`Workstation` чи `Server`, неконкурентний чи фоновий).
