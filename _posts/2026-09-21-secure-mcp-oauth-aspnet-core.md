...
Для протоколу `2026-07-28` сирий HTTP-запит також містить `Mcp-Method`, `Mcp-Name` та `_meta` рівня запиту з версією протоколу, client info і capabilities. Це не декоративні заголовки: офіційний SDK 2.0.0 відхиляє неповний запит. Тестування через реальний transport допомагає помітити таку зміну раніше, ніж production client перестане підключатися.
...
Bearer token сам по собі не робить MCP server безпечним. Надійна межа складається з кількох перевірок, кожна з яких відповідає на окреме питання:

- PKCE: чи може той самий client обміняти authorization code
- `resource` і `aud`: чи призначений token саме цьому server
- scope: чи дозволений клас операції
- client entitlement: чи може цей client просити такий scope
- перевірка tenant і власника: чи доступний конкретний об'єкт користувачеві
- audit: чи зможемо відновити подію після інциденту
- rate limiting: чи не перетвориться легальний доступ на вичерпання ресурсів
